...
public class Lane extends Thread implements PinsetterObserver {
	private LaneProduct laneProduct = new LaneProduct();
	private Party party;
	...
	public Lane() {
		...
		laneProduct.setScores(new HashMap<Bowler, int[]>());
		...
		laneProduct.setCurrentCumulScores(new ScoreCount(bowlIndex));
		this.start();
	}
	public void run() {
		while (true) {
			if (partyAssigned && !gameFinished) {	// we have a party on this lane,
				...
				if (bowlerIterator.hasNext()) {
					...
					if (frameNumber == 9){
						finalScores[bowlIndex][gameNumber] = laneProduct.getCurrentCumulScores().getFinalScore();
						try{
							Date date = new Date();
							String dateString = "" + date.getHours() + ":" + date.getMinutes() + " " + date.getMonth() + "/" + date.getDay() + "/" + (date.getYear() + 1900);
							int finalscore=laneProduct.getCurrentCumulScores().getFinalScore();
					...
					setter.reset();
					bowlIndex++;
					laneProduct.getCurrentCumulScores().setBowlIndex(bowlIndex);
	...
	public void receivePinsetterEvent(PinsetterEvent pe) {
		...
		if (pe.pinsDownOnThisThrow() >=  0) {			// this is a real throw
			laneProduct.markScore(currentThrower, frameNumber + 1, pe.getThrowNumber(), pe.pinsDownOnThisThrow(), this);
	...
	private void resetScores() {
		...
		while ( bowlIt.hasNext() ) {
			...
			laneProduct.getScores().put( bowlIt.next(), toPut );
		}
		gameFinished = false;
		frameNumber = 0;
	}
	...
	public void assignParty( Party theParty ) {
		...
		laneProduct.getCurrentCumulScores().reset(party.getMembers().size());
	...
	public void publish() {
		if( subscribers.size() > 0 ) {
			...
			while ( eventIterator.hasNext() ) {
				( (LaneObserver) eventIterator.next()).receiveLaneEvent( party, bowlIndex, currentThrower, laneProduct.getCurrentCumulScores().getCumulScores(), laneProduct.getScores(), frameNumber+1, curScores, ball, gameIsHalted );
			}
		}
	}
	...
	public Object clone() throws java.lang.CloneNotSupportedException {
		Lane clone = (Lane) super.clone();
		clone.laneProduct = (LaneProduct) this.laneProduct.clone();
		return clone;
	}
